...
fallback = absent
current extension = Safe Canonical Refresh / Operational Orchestration v0.1 / Scientific Entity GLiNER Held-Out Evaluation v0.1
dataset publication = paused pending redistribution guidance
next accepted functional direction = structured held-out scientific-entity error analysis and one bounded extractor v0.2 hypothesis
```
...
scientific_entity_real_review_complete = true_bounded_local_pilot
scientific_entity_real_review_id = scientific-entity-manual-review-v0.1-20260821T131320262656Z
scientific_entity_real_review_documents = 24
scientific_entity_real_review_annotation_rows = 48
scientific_entity_real_review_reference_mentions = 435
scientific_entity_literal_candidate_build_id = scientific-entity-literal-v0.1-20260822T114316573133Z
scientific_entity_literal_candidate_predictions = 30
scientific_entity_pilot_evaluation_id = scientific-entity-evaluation-v0.1-20260822T114935748579Z
scientific_entity_pilot_evaluation_validator_checks = 69 / 69
...
scientific_entity_gliner_calibration_tooling_status = implemented_fixture_validated
scientific_entity_gliner_calibration_declared_trials = 127
scientific_entity_gliner_calibration_fixture_eligible_trials = 70
scientific_entity_gliner_calibration_fixture_pareto_trials = 14
scientific_entity_gliner_calibration_fixture_validator_checks = 53 / 53
scientific_entity_gliner_real_calibration_status = completed_candidate_strict_validated
scientific_entity_gliner_real_calibration_id = scientific-entity-gliner-dev-calibration-v0.1-20260823T152930597192Z
scientific_entity_gliner_real_calibration_eligible_trials = 69
scientific_entity_gliner_real_calibration_pareto_trials = 29
scientific_entity_gliner_real_calibration_validator_checks = 53 / 53
scientific_entity_gliner_selected_dev_profile = balanced_f1
scientific_entity_gliner_selected_dev_trial_id = calibration-trial:1172aea9d875d59f3b39cc21488dec8f
scientific_entity_gliner_selected_title_threshold = 0.55
scientific_entity_gliner_selected_abstract_threshold = 0.65
scientific_entity_gliner_selected_entity_type_overrides = none
scientific_entity_gliner_selected_exact_f1 = 0.380146
scientific_entity_gliner_selected_relaxed_f1 = 0.404358
scientific_entity_gliner_dev_policy_frozen = true
scientific_entity_gliner_type_probe_policy_selection = diagnostic_only
scientific_entity_gliner_scores_reinterpreted_as_probabilities = false
scientific_entity_heldout_review_id = scientific-entity-heldout-review-v0.1-20260827T092900455472Z
scientific_entity_heldout_documents = 48
scientific_entity_heldout_references = 881
scientific_entity_heldout_reference_validator_checks = 4444 / 4444
scientific_entity_heldout_raw_build_id = scientific-entity-gliner-small-v2.5-v0.1-20260827T111030652864Z
scientific_entity_heldout_raw_predictions = 1145
scientific_entity_heldout_raw_build_validator_checks = 91 / 91
scientific_entity_heldout_policy_build_id = scientific-entity-gliner-small-v2.5-heldout-frozen-policy-v0.1-20260827T112658493807Z
scientific_entity_heldout_policy_selected_predictions = 787
scientific_entity_heldout_policy_rejected_predictions = 358
scientific_entity_heldout_policy_validator_checks = 4762 / 4762
scientific_entity_heldout_evaluation_id = scientific-entity-evaluation-v0.1-20260827T113112815887Z
scientific_entity_heldout_evaluation_validator_checks = 69 / 69
scientific_entity_heldout_exact_f1 = 0.396882
scientific_entity_heldout_relaxed_f1 = 0.414868
scientific_entity_heldout_generalization_gate = passed
scientific_entity_bounded_working_extractor_accepted = true
scientific_entity_current_dev_set_becomes_held_out = false
...
→ Scientific Entity Literal Baseline Pilot / real bounded descriptive evaluation evidence
→ retrieval artifacts
...
Bounded Scientific Entity Manual Review Evidence = local prediction-blind reference-evidence tooling and completed pilot reference package; not model acceptance
Scientific Entity Literal Baseline Pilot = bounded dev-quality evidence; not production selection or full-corpus authorization
...
retained without production selection. A read-only 127-trial calibration layer
now searches fixed global and source-field thresholds, emits F0.5/F1/F2 and
Pareto dev recommendations, keeps per-type probes diagnostic, and reproduces
all output bytes without model inference. Its synthetic fixture is green; the
next accepted action is local execution and review on the existing 24-paper dev
package. Prompt/filter changes require a new immutable candidate build. New
disjoint prediction-blind evidence follows only after one policy is frozen.
...
Evidence v0.1**, described below. Its first real bounded execution is now
complete, but no production model or full-corpus build is authorized without a
separate candidate and held-out acceptance decision.
...
The first real operational execution is complete. It ran the accepted tooling
against current canonical latest, completed the 24-paper/48-row review
prediction-blind, produced 435 reference mentions, validated the immutable
completed package, and used the existing baseline/evaluation harness.

## Scientific Entity Literal Baseline Pilot Evaluation v0.1

The tracked aggregate checkpoint is:

```text
docs/scientific_entity_literal_baseline_pilot_evaluation_v0.1.md
```

The local derived chain is:
...
24 papers / 48 annotation rows / 435 references
30 literal predictions / 10 exact matches / 6 relaxed-only matches
17 boundary mismatches / 2 type mismatches / 1 false positive / 406 false negatives
independent evaluation validator = 69 / 69
```

The result establishes the literal implementation as an executable control, not
as a quality candidate:

```text
exact precision / recall / F1 = 0.333333 / 0.022989 / 0.043012
relaxed precision / recall / F1 = 0.533333 / 0.036782 / 0.068818
all six per-type support markers = sufficient
document count = 24 / configured minimum 32
...
## Bounded Scientific Entity GLiNER Dev Calibration v0.1 — tooling implemented

The implemented fixed-prediction slice uses this bounded order:

```text
unfiltered baseline metric recomputation
→ 9 global threshold trials
→ 63 title/abstract threshold-pair trials
→ exact F0.5/F1/F2 profile and exact precision/recall Pareto evidence
→ 54 isolated per-type probes over the balanced source-pair base
→ no combined per-type policy selection
→ local execution and human dev-policy review
...
No model inference or model download occurs in this layer; input scores remain
`model_score`, not calibrated probabilities. Prompt variants and deterministic
generic/cross-sentence rules require a separate immutable prediction build and
new extractor fingerprint. A second-stage classifier remains deferred until
those cheaper controls leave a measured residual bottleneck. Any adjudicated
dev correction or tuned configuration receives a new identity. No calibration
result on these 24 papers may be presented as held-out evidence.

The real candidate execution is now complete and independently validated. Human
review freezes the balanced source-field policy `title >= 0.55` /
`abstract >= 0.65` with no entity-type overrides, referenced by calibration
`scientific-entity-gliner-dev-calibration-v0.1-20260823T152930597192Z` and trial `calibration-trial:1172aea9d875d59f3b39cc21488dec8f`. The policy is development-only. The original
546-mention candidate build remains immutable; the next semantic materialization
must receive a new candidate identity before disjoint held-out review.
