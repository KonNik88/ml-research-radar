...
Refresh Preflight Contract v0.1 — 2026-08 completed read-only refresh-start validator before controlled corpus expansion
```
...
refresh_preflight_contract_v0.1 = read_only_refresh_start_gate
...
Before running a real refresh or controlled corpus expansion, run the read-only
preflight validator:

```bat
python -m scripts.validation.check_refresh_preflight_contract --strict
```

For the full refresh-start gate, use:
...
no retrieval, Qdrant, topic, graph, ranking, API, or UI artifact is rebuilt
