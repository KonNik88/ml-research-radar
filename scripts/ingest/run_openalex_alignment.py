...
def fetch_openalex_by_doi(
    ...
    *,
    ...
    # Для doi-filter обычно достаточно первой записи
    return results[0]


def build_match_row(arxiv_row: dict[str, Any], openalex_entry: Optional[dict[str, Any]], *, error: Optional[str] = None) -> dict[str, Any]:
...
def render_markdown(report: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# OpenAlex alignment ingest from arXiv DOIs")
    lines.append("")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch OpenAlex enrichment slice aligned to latest arXiv normalized corpus by DOI."
    )
    parser.add_argument(
        "--arxiv",
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Maximum number of unique arXiv DOIs to request from OpenAlex.",
    )
    parser.add_argument(
        "--mailto",
        default=None,
...
def main() -> None:
    ...
    manifest = {
        ...
        "arxiv_input_file": str(arxiv_path),
    ...
    print(f"[OK] arXiv total docs: {len(arxiv_rows)}")
