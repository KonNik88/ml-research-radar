"""Validate the Paper-Artifact Graph manual-review checklist gate.

This validator is intentionally read-only. It checks that the manual-review
checklist for the local Paper-Artifact Graph v0.1 package is structurally valid,
that the completed line checkpoint and package manifest remain safe, and that
publication is correctly blocked until manual review is completed.
...
truth, touch Postgres/Qdrant/retrieval/ranking/API/UI, create a runtime graph, or
publish anything.
"""
...
SCHEMA_VERSION = "paper_artifact_graph_manual_review_v1"
CONFIG_SCHEMA_VERSION = "paper_artifact_graph_manual_review_config_v1"
DEFAULT_CONFIG_PATH = Path("configs/paper_artifact_graph_manual_review.yaml")
...
def line_checkpoint_report_green(path: Path) -> tuple[bool, dict[str, Any]]:
    ...
    ok = (
        ...
        and verdict.get("paper_artifact_graph_line_complete") is True
        and verdict.get("local_checkpoint_ready") is True
        and verdict.get("manual_review_required_before_publication") is True
    ...
    return ok, {
        ...
        "paper_artifact_graph_line_complete": verdict.get("paper_artifact_graph_line_complete"),
        "local_checkpoint_ready": verdict.get("local_checkpoint_ready"),
        "manual_review_required_before_publication": verdict.get("manual_review_required_before_publication"),
...
def package_manifest_safety(path: Path) -> tuple[bool, dict[str, Any]]:
    ...
    ok = manifest.get("schema_version") == "paper_artifact_graph_package_manifest_v1" and package_ok and not boundary_mismatches
    return ok, {
        "path": normalize_path(path),
        "exists": True,
...
def validate_manual_review(
    *,
    ...
    review_expected = {
        "name": "paper_artifact_graph_manual_review",
        "version": "v0.1",
...
def write_validation_reports(result: dict[str, Any], report_dir: Path) -> None:
    ...
    latest_json = report_dir / "paper_artifact_graph_manual_review_latest.json"
    latest_md = report_dir / "paper_artifact_graph_manual_review_latest.md"
    history_json = history_dir / f"paper_artifact_graph_manual_review_{run_ts}.json"
    history_md = history_dir / f"paper_artifact_graph_manual_review_{run_ts}.md"
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate the Paper-Artifact Graph manual-review checklist gate.")
