"""Validate the local Paper-Artifact Graph package output.

This validator checks package-level integrity after
scripts.export.package_paper_artifact_graph has created the local package.
...
SCHEMA_VERSION = "paper_artifact_graph_package_validation_v1"
...
def actual_graph_counts_from_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    quality = manifest.get("graph", {}).get("quality_summary", {})
...
def render_markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        "# Paper-Artifact Graph Package Check",
        "",
...
def write_validation_reports(result: dict[str, Any], report_dir: Path) -> None:
    ...
    latest_json = report_dir / "paper_artifact_graph_package_latest.json"
    latest_md = report_dir / "paper_artifact_graph_package_latest.md"
    history_json = history_dir / f"paper_artifact_graph_package_{run_ts}.json"
    history_md = history_dir / f"paper_artifact_graph_package_{run_ts}.md"
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate local Paper-Artifact Graph package output.")
