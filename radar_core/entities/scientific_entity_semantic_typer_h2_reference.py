...
REPORT_NAME = "scientific_entity_semantic_typer_h2_reference_v03"
FROZEN_FILES = {
...
def _read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ScientificEntityH2ReferenceError(f"Expected JSON object: {path}")
    return payload
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, raw_line in enumerate(handle, start=1):
            if not raw_line.strip():
                raise ScientificEntityH2ReferenceError(
...
def _normalize_annotator_ids(values: Sequence[str]) -> list[str]:
    ...
    if len(set(normalized)) != len(normalized):
        raise ScientificEntityH2ReferenceError("annotator_id values must be unique")
    return normalized
...
def _validate_parent_state(
    *,
    project_root: Path,
    config_path: Path,
    acceptance_gate_config_path: Path,
    sample_dir: Path,
    canonical_path: Path,
    ...
    config = load_h2_reference_config(config_path)

    gate = load_h2_independent_acceptance_gate_config(acceptance_gate_config_path)
    gate_sha = acceptance_gate_canonical_sha256(gate)
    if gate_sha != config.acceptance_gate.expected_canonical_config_sha256:
        raise ScientificEntityH2ReferenceError(
            "independent acceptance gate config drifted from frozen reference tooling"
        )
    if gate_sha != ACCEPTANCE_GATE_CONFIG_SHA256:
        raise ScientificEntityH2ReferenceError(
            "independent acceptance gate canonical SHA-256 does not match frozen constant"
        )
    if gate.candidate_lineage.candidate_id != config.candidate.candidate_id:
        raise ScientificEntityH2ReferenceError(
            "acceptance gate candidate_id does not match reference tooling"
        )
    if (
        ...
        raise ScientificEntityH2ReferenceError(
            "acceptance gate candidate fingerprint does not match reference tooling"
        )
    if gate.fresh_heldout_lineage.sample_id != config.sample.sample_id:
        raise ScientificEntityH2ReferenceError(
            "acceptance gate sample_id does not match reference tooling"
        )
    if gate.fresh_heldout_lineage.review_id != config.sample.review_id:
        raise ScientificEntityH2ReferenceError(
            "acceptance gate review_id does not match reference tooling"
        )

    sample_dir = sample_dir.resolve()
    manifest_path = sample_dir / "manifest.json"
    manifest = H2FreshHeldoutSampleManifest.model_validate(_read_json(manifest_path))
    if sha256_file(manifest_path) != config.sample.expected_sample_manifest_sha256:
        raise ScientificEntityH2ReferenceError(
            "fresh H2 sample manifest SHA-256 does not match frozen reference tooling"
        )
    if manifest.sample_id != config.sample.sample_id:
        raise ScientificEntityH2ReferenceError("sample_id drifted")
    if manifest.review_id != config.sample.review_id:
        raise ScientificEntityH2ReferenceError("review_id drifted")
    if manifest.candidate_id != config.candidate.candidate_id:
        raise ScientificEntityH2ReferenceError("sample candidate_id drifted")
    if (
        manifest.candidate_fingerprint_sha256
        != config.candidate.candidate_fingerprint_sha256
    ):
        raise ScientificEntityH2ReferenceError("sample candidate fingerprint drifted")
    if manifest.selected_document_count != config.sample.expected_document_count:
        raise ScientificEntityH2ReferenceError("sample document count drifted")
    if manifest.annotation_row_count != config.sample.expected_annotation_row_count:
        raise ScientificEntityH2ReferenceError("sample annotation row count drifted")
    if (
        manifest.excluded_consumed_union_document_count
        != config.sample.expected_consumed_union_document_count
    ):
        raise ScientificEntityH2ReferenceError("sample consumed-union count drifted")
    if (
        manifest.sample_consumed_union_overlap_count
        != config.sample.expected_consumed_union_overlap_count
    ):
        raise ScientificEntityH2ReferenceError("sample consumed-union overlap drifted")
    ...
    if not manifest.prediction_blind or not manifest.annotations_initially_empty:
        raise ScientificEntityH2ReferenceError(
            "fresh H2 sample must remain prediction-blind with initially empty annotations"
        )
    forbidden_true = {
    ...
    if any(forbidden_true.values()):
        raise ScientificEntityH2ReferenceError(
            f"fresh H2 sample safety flags drifted: {forbidden_true}"
        )
    ...
    checks, summary = validate_h2_fresh_heldout_sample(
        ...
        config_path=(
            project_root
            / "configs"
            / "scientific_entity_semantic_typer_h2_fresh_heldout_sample_v0.3.yaml"
        ).resolve(),
    ...
    if any(not ok for _, ok, _ in checks) or summary["required_failed_count"] != 0:
        raise ScientificEntityH2ReferenceError(
            "independent H2 fresh held-out sample validation failed"
        )
    return config, manifest, selected_sha, gate_sha, summary
...
def prepare_annotation_working_copy(
    *,
    ...
    config, manifest, selected_sha, gate_sha, _ = _validate_parent_state(
        project_root=project_root.resolve(),
        config_path=config_path.resolve(),
        acceptance_gate_config_path=acceptance_gate_config_path.resolve(),
        sample_dir=sample_dir.resolve(),
        canonical_path=canonical_path.resolve(),
        development_package_dir=development_package_dir.resolve(),
        previous_heldout_sample_dir=previous_heldout_sample_dir.resolve(),
    ...
    if len(blank_rows) != config.sample.expected_annotation_row_count:
        raise ScientificEntityH2ReferenceError("blank annotation row count drifted")
    ...
    readme = (
        "# H2 v0.3 prediction-blind annotation working copy\n\n"
        f"Candidate ID: `{config.candidate.candidate_id}`\n\n"
        f"Candidate fingerprint: `{config.candidate.candidate_fingerprint_sha256}`\n\n"
        ...
        "Do not inspect or run baseline/H1/H2 predictions before reference freeze.\n"
        "Do not tune H2 threshold/policy from this held-out.\n\n"
        f"Frozen selected canonical IDs SHA-256: `{selected_sha}`\n\n"
        f"Frozen independent acceptance gate canonical SHA-256: `{gate_sha}`\n"
    ).encode("utf-8")
    ...
    return {
        ...
        "independent_acceptance_gate_config_sha256": gate_sha,
        ...
        "next_slice": "complete_prediction_blind_h2_manual_annotations",
    }
...
def _compute_frozen_reference(
    *,
    ...
    config, sample_manifest, selected_sha, gate_sha, sample_summary = _validate_parent_state(
        project_root=project_root.resolve(),
        config_path=config_path.resolve(),
        acceptance_gate_config_path=acceptance_gate_config_path.resolve(),
        sample_dir=sample_dir.resolve(),
    ...
    exclusion_path = sample_dir / "exclusion_provenance.json"
    manifest_path = sample_dir / "manifest.json"
    ...
    audit = {
        ...
        "excluded_consumed_union_document_count": 120,
        "sample_consumed_union_overlap_count": 0,
        "document_count": 48,
    ...
    completion_manifest = ScientificEntityH2ReferenceCompletionManifest(
        ...
        exclusion_provenance_file="exclusion_provenance.json",
        exclusion_provenance_sha256=sha256_file(exclusion_path),
        ...
        redistribution_allowed=False,
        publication_ready=False,
        next_slice="run_frozen_h2_independent_inference",
    )
    completion_bytes = _json_bytes(completion_manifest.model_dump(mode="json"))

    readme = (
        "# Scientific Entity H2 v0.3 reference evidence\n\n"
        f"Candidate ID: `{config.candidate.candidate_id}`\n\n"
        f"Candidate fingerprint: `{config.candidate.candidate_fingerprint_sha256}`\n\n"
        f"Sample ID: `{config.sample.sample_id}`\n\n"
        f"Review ID: `{config.sample.review_id}`\n\n"
        "This immutable package contains prediction-blind manually adjudicated "
        "reference evidence for the new disjoint H2 v0.3 independent held-out sample.\n\n"
        "- annotation rows: 96\n"
        f"- reference mentions: {len(references)}\n"
        "- uncertain mentions: 0\n"
        f"- minimum references per type: {minimum}\n"
        "- sample independent validation: passed\n"
        "- overlap with 120 consumed documents: 0\n"
        "- H2 model inference executed: false\n"
        "- evaluation executed: false\n"
        "- threshold/policy tuning executed: false\n"
...
def freeze_reference_evidence(
    *,
    ...
    files, report = _compute_frozen_reference(
        ...
        acceptance_gate_config_path=acceptance_gate_config_path,
        sample_dir=sample_dir,
    ...
    return {
        **report,
        ...
        "output_dir": str(output_dir).replace("\\", "/"),
        "next_slice": (
            "validate_h2_v03_reference_evidence"
            if execute
            else "execute_h2_v03_reference_freeze"
        ),
    }
...
def validate_frozen_reference_evidence(
    *,
    ...
    add(
        "sample_independent_validation_passed",
        sample_summary["required_failed_count"] == 0,
        sample_summary["required_failed_count"],
    )

    completion = ScientificEntityH2ReferenceCompletionManifest.model_validate(
    ...
    add(
        ...
        recomputed_reference_bytes
        == (reference_dir / "reference_mentions.jsonl").read_bytes(),
        len(references),
    )
    ...
    add(
        "all_annotation_rows_complete",
        all(row.annotation_complete for row in completed_rows),
        sum(int(row.annotation_complete) for row in completed_rows),
    )
    ...
    add(
        ...
        completion.candidate_id == config.candidate.candidate_id == sample_manifest.candidate_id,
        completion.candidate_id,
    )
    add(
        "candidate_fingerprint_matches",
        completion.candidate_fingerprint_sha256
        == config.candidate.candidate_fingerprint_sha256
        == sample_manifest.candidate_fingerprint_sha256,
        completion.candidate_fingerprint_sha256,
    )
    add(
        "acceptance_gate_sha_matches",
        completion.independent_acceptance_gate_config_sha256 == gate_sha,
        completion.independent_acceptance_gate_config_sha256,
    )
    add(
        "sample_id_matches",
        completion.sample_id == sample_manifest.sample_id == config.sample.sample_id,
        completion.sample_id,
    )
    add(
        "review_id_matches",
        completion.review_id == review.review_id == config.sample.review_id,
        completion.review_id,
    )
    add(
        "selected_ids_sha_matches",
        completion.selected_canonical_ids_sha256 == selected_sha,
        completion.selected_canonical_ids_sha256,
    )
    add(
        "sample_manifest_sha_matches",
        completion.sample_manifest_sha256
        == sha256_file(sample_dir.resolve() / "manifest.json"),
        completion.sample_manifest_sha256,
    )
    add(
        "blank_sha_matches",
        completion.blank_annotations_sha256
        == sha256_file(sample_dir.resolve() / "annotations_working.jsonl"),
        completion.blank_annotations_sha256,
    )
    add(
        "canonical_sample_sha_matches",
        completion.canonical_sample_sha256
        == sha256_file(sample_dir.resolve() / "canonical_documents.sample.jsonl"),
        completion.canonical_sample_sha256,
    )
    add(
        "sample_assignments_sha_matches",
        completion.sample_assignments_sha256
        == sha256_file(sample_dir.resolve() / "sample_assignments.jsonl"),
        completion.sample_assignments_sha256,
    )
    add(
        "exclusion_provenance_sha_matches",
        completion.exclusion_provenance_sha256
        == sha256_file(sample_dir.resolve() / "exclusion_provenance.json"),
        completion.exclusion_provenance_sha256,
    )
    add(
        "completed_sha_matches",
        completion.completed_annotations_sha256
        == sha256_file(reference_dir / "completed_annotations.jsonl"),
        completion.completed_annotations_sha256,
    )
    add(
        "review_manifest_sha_matches",
        completion.review_manifest_sha256
        == sha256_file(reference_dir / "review_manifest.json"),
        completion.review_manifest_sha256,
    )
    add(
        "reference_sha_matches",
        completion.reference_mentions_sha256
        == sha256_file(reference_dir / "reference_mentions.jsonl")
    ...
    add(
        ...
        == sha256_file(reference_dir / "annotation_audit_summary.json"),
        completion.annotation_audit_sha256,
    )
    add(
        "reference_count_matches",
        completion.reference_mention_count == review.reference_mention_count == len(references),
        len(references),
    )
    add(
        "reference_adequacy_passed",
        completion.reference_adequacy_passed
        and audit.get("reference_adequacy_passed") is True,
        "",
    )
    add(
        "prediction_blind",
    ...
    add("reference_frozen", completion.reference_frozen is True, "")
    ...
    safety_ok = (
        ...
        and completion.h2_model_inference_executed is False
        and completion.evaluation_executed is False
        and completion.threshold_tuning_executed is False
        and completion.policy_revision_executed is False
    ...
    add(
        "next_slice_run_frozen_h2",
        completion.next_slice == "run_frozen_h2_independent_inference",
        completion.next_slice,
    )
    ...
    summary = {
        ...
        "candidate_fingerprint_sha256": completion.candidate_fingerprint_sha256,
        "independent_acceptance_gate_config_sha256": completion.independent_acceptance_gate_config_sha256,
        ...
        "reference_frozen": completion.reference_frozen,
        "h2_model_inference_executed": completion.h2_model_inference_executed,
        "evaluation_executed": completion.evaluation_executed,
        "threshold_tuning_executed": completion.threshold_tuning_executed,
        "policy_revision_executed": completion.policy_revision_executed,
