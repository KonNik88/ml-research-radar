...
def test_scientific_entity_v03_typing_diagnostics_preparation_is_current() -> None:
    ...
    assert "references = 944" in evaluation
    assert "predictions = 773" in evaluation
    assert "exact precision = 0.443726" in evaluation
    assert "exact recall = 0.363347" in evaluation
    assert "exact F1 = 0.399534" in evaluation
    assert "relaxed precision = 0.472186" in evaluation
    assert "relaxed recall = 0.386653" in evaluation
    assert "relaxed F1 = 0.42516" in evaluation
    assert "model -> method = 70" in evaluation
    assert "method -> task = 15" in evaluation
    assert "total type mismatch = 166" in evaluation
    assert "method predicted-type mismatch sink = 84" in evaluation
    assert "maximum predicted-type mismatch sink = method:84" in evaluation
    assert "base evaluation validation = 69 / 69" in evaluation
    assert "wrapper strict validation = 19 / 19" in evaluation
    assert "evaluation executed = true" in evaluation
    ...
    assert "raw prediction count = 1257" in policy
    assert "selected prediction count = 773" in policy
    assert "rejected prediction count = 484" in policy
    ...
    assert "policy extractor fingerprint = 77af105871b227daa0d8c9e5501839addf229004795490a63bebe4f02672cf52" in policy
    assert "model inference executed by policy = false" in policy
    assert "policy applied = true" in policy
    assert "threshold tuning executed = false" in policy
    assert "reference labels used for filtering = false" in policy
    assert "evaluation executed = false" in policy
    assert "acceptance decision made = false" in policy
    assert "total checks = 46" in policy
    assert "required failures = 0" in policy
    assert "next = evaluate_frozen_v02c_on_fresh_heldout_once" in policy
